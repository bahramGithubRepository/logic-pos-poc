...
def test_max_defrost_and_recirculation_dry_run(hil_config):
    """
    DRY RUN: Verify Max Defrost Activates, Recirculation Turns Off
    ...
    reporter = TestReporter(
        "Max Defrost Activates, Recirculation Turns Off - DRY RUN",
        "Verify that while Max Defrost is activated, the Thermal System uses outside air only by ensuring air recirculation is turned off."
    )

    print("\n" + "="*70)
    print("[DRY RUN] MAX DEFROST AND RECIRCULATION TEST")
    print("="*70)
    ...
    signals_to_read = [
        ...
        # Add other relevant status signals if needed for pre-condition verification
    ]
    ...
    print("-" * 70)
    reporter.add_step("Step 1: Set Pre-Conditions", "Set initial state: Max Defrost inactive, Recirculation active, Vehicle Running.")

    asyncio.run(asyncio.sleep(0.5))

    set_can_signal(hil_var, "VehicleMode", 6) # Vehicle is in "Running" mode (as per template)
    set_can_signal(hil_var, "MaxDefrostRequest", 0) # Max Defrost is inactive.
    set_can_signal(hil_var, "AirRecirculationRequest", 1) # Air recirculation is active.

    print("\n[STEP 2] Verify Pre-Conditions (DRY RUN)...")
    print("-" * 70)
    reporter.add_step("Step 2: Verify Pre-Conditions", "Simulate expected responses to pre-conditions.")
    ...
    # ========================================================================
    # STEPS
    # 1. Set MaxDefrostStatus to 0. (Already covered in pre-conditions)
    # 2. Set AirRecirculationStatus to 1. (Already covered in pre-conditions)
    # 3. Set MaxDefrostStatus to 1. (Trigger)
    # ========================================================================
    print("\n[STEP 3] Trigger: Activate Max Defrost (DRY RUN)...")
    print("=" * 70)
    reporter.add_step("Step 3: Activate Max Defrost", "Driver requests activation of Max Defrost.")

    set_can_signal(hil_var, "MaxDefrostRequest", 1)

    print("\n[STEP 4] Waiting for system response (DRY RUN)...")
    print("-" * 70)
    reporter.add_step("Step 4: Wait for System Response", "Allow time for the system to react to Max Defrost activation.")
    ...
    print("-" * 70)
    reporter.add_step("Step 5: Verify Expected Outcome", "Check if Max Defrost is active and Air Recirculation is off.")
    ...
    # Generate report
    report_path = reporter.generate_html("test_max_defrost_recirculation_dry_run_report.html")

    print(f"\nDry Run Report: {report_path}")
    print("\n[!] To run for REAL:")
    print("   1. Review the dry run report")
    print("   2. Verify all signals are correct")
    print("   3. Run: pytest -v test_max_defrost_recirculation.py")
    print("="*70 + "\n")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*70)
    print("[DRY RUN] MAX DEFROST AND RECIRCULATION DRY RUN")
    print("="*70)
    print("\nThis will:")
    ...
    _, _, _, hil_var = read_project_config()
    test_max_defrost_and_recirculation_dry_run(hil_var)