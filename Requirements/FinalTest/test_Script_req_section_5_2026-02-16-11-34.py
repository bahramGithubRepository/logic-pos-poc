...
def test_max_defrost_activation_dry_run(hil_config):
    """
    DRY RUN: Verify Max Defrost activation and status reporting
    """
    ...
    # ========================================================================
    # PRE-CONDITIONS
    # - Vehicle in Running mode (VehicleMode = 6)
    # - Max Defrost is currently inactive (MaxDefrostRequest = 0, MaxDefrostStatus = 0)
    # - Set other signals to a known state (e.g., Floor and Vent ON, Blower Low, Recirculation ON)
    # ========================================================================
    print("\n[STEP 1] Setting Pre-Conditions (DRY RUN)...")
    print("-" * 70)
    reporter.add_step("Step 1: Set Pre-Conditions (DRY RUN)", "Ensure vehicle is in Running mode and Max Defrost is inactive")
    ...
    set_can_signal(hil_var, "ClimateAirDistRequest_Floor", 1) # Floor ON
    set_can_signal(hil_var, "ClimateAirDistRequest_Vent", 1) # Vent ON
    set_can_signal(hil_var, "AirRecirculationRequest", 1) # Recirculation ON
    set_can_signal(hil_var, "HVACBlowerRequest", 3) # Low Blower
    set_can_signal(hil_var, "CabHeatManReq", 0) # Heater OFF
    ...
    checks_passed &= check_can_signal(hil_var, "HVACBlowerLevelStat_BlowerLevel", 3)
    ...
    # ========================================================================
    print("\n[STEP 3] Trigger: Request Max Defrost Activation (DRY RUN)...")
    print("=" * 70)
    reporter.add_step("Step 3: Activate Max Defrost Request", "Set MaxDefrostRequest to 1")
    
    set_can_signal(hil_var, "MaxDefrostRequest", 1)
    
    print("\n[STEP 4] Waiting for system response (DRY RUN)...")
    print("-" * 70)
    reporter.add_step("Step 4: Wait for System Response", "Allow time for system to react (simulated)")
    
    asyncio.run(asyncio.sleep(0.5)) # Wait for the system to process the request
    ...
    # - AirRecirculationStatus == 0 (Fresh Air)
    # - HVACBlowerLevelStat_BlowerLevel == 10 (Highest Blower Speed)
    # - CabHeatManStatus == 10 (Highest Heating)
    # - ClimateAirDistStatus_Defrost == 1 (Defrost ON)
    # ========================================================================
    print("\n[STEP 5] Verify Expected Outcome (DRY RUN)...")
    print("=" * 70)
    reporter.add_step("Step 5: Verify Max Defrost Activation and Effects", "Check all relevant status signals")
    
    checks_passed_step5 = True
    checks_passed_step5 &= check_can_signal(hil_var, "MaxDefrostStatus", 1)
    checks_passed_step5 &= check_can_signal(hil_var, "ClimateAirDistStatus_Floor", 0)
    checks_passed_step5 &= check_can_signal(hil_var, "ClimateAirDistStatus_Vent", 0)
    checks_passed_step5 &= check_can_signal(hil_var, "AirRecirculationStatus", 0)
    checks_passed_step5 &= check_can_signal(hil_var, "HVACBlowerLevelStat_BlowerLevel", 10)
    checks_passed_step5 &= check_can_signal(hil_var, "CabHeatManStatus", 10)
    checks_passed_step5 &= check_can_signal(hil_var, "ClimateAirDistStatus_Defrost", 1)
    
    # Update overall checks_passed
    checks_passed &= checks_passed_step5
    ...
    print("   3. Run: pytest -v test_max_defrost_activation_safe.py")
    print("="*70 + "\n")
