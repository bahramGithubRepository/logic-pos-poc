...
def simulate_hardware_response(signal_name, expected_value):
    ...
    response_map = {
        ...
        # Cabin heater - in max defrost, heat level changes are allowed.
...
def test_max_defrost_heat_level_change(hil_config):
    """
    DRY RUN: Test validation for Max Defrost Heat Level Change scenario without hardware control
    """

    global reporter
    reporter = TestReporter(
        "Max Defrost Heat Level Change - DRY RUN",
        "Verify that when Max Defrost is active and the Driver Experience System has changed the Heat Level setting (Except to LO/MIN), the Thermal System shall change Heat Level setting accordingly, keep Max Defrost active and store Heat Level setting."
    )

    print("\n" + "="*70)
    print("[DRY RUN] MAX DEFROST HEAT LEVEL CHANGE TEST")
    print("="*70)
    ...
    # ========================================================================
    print("\n[STEP 1] Setting Pre-conditions: Max Defrost Active & Vehicle Running")
    print("-" * 70)
    reporter.add_step(
        "Step 1: Set Pre-conditions",
        "Set VehicleMode to 'Running' and activate Max Defrost. Ensure current heat level is not LO/MIN."
    )

    # Pre-conditions:
    # - Vehicle is in 'Running' mode. (VehicleMode = 6 based on template)
    # - Max Defrost is active. (MaxDefrostRequest = 1)
    set_can_signal(hil_var, "VehicleMode", 6)
    set_can_signal(hil_var, "MaxDefrostRequest", 1)
    # Ensure CabHeatManReq is at a default non-LO/MIN value if not already set, for consistent state
    set_can_signal(hil_var, "CabHeatManReq", 8) # Intermediate heat level

    asyncio.run(asyncio.sleep(0.5))

    # Verify Pre-conditions
    print("\n[STEP 1 - Verification] Verify Pre-conditions:")
    reporter.add_step(
        "Step 1 - Verification: Pre-conditions",
        "Verify MaxDefrostStatus is ON and vehicle is in Running mode, and appropriate air distribution."
    )
    checks_passed = True
    ...
    checks_passed &= check_can_signal(hil_var, "ClimateAirDistStatus_Floor", 0) # Negative check
    checks_passed &= check_can_signal(hil_var, "ClimateAirDistStatus_Vent", 0)   # Negative check
    checks_passed &= check_can_signal(hil_var, "CabHeatManStatus", 8) # Verify initial heat level is reflected
    # Check other Max Defrost related signals that should be in specific states
    checks_passed &= check_can_signal(hil_var, "AirRecirculationStatus", 0) # Fresh air only
    checks_passed &= check_can_signal(hil_var, "HVACBlowerLevelStat_BlowerLevel", 10) # Highest blower level

    # ========================================================================
    # STEP 2: Trigger - Change CabHeatManReq
    # ========================================================================
    print("\n[STEP 2] Trigger: Change CabHeatManReq to 5")
    print("-" * 70)
    reporter.add_step(
        "Step 2: Trigger Heat Level Change",
        "Driver Experience System changes CabHeatManReq to an intermediate value (5)."
    )

    set_can_signal(hil_var, "CabHeatManReq", 5)

    # ========================================================================
    # STEP 3: Wait for system to process
    # ========================================================================
    print("\n[STEP 3] Waiting for system to process changes (3 seconds)")
    print("-" * 70)
    reporter.add_step(
        "Step 3: Wait for Processing",
        "Allow sufficient time for the system to process the new heat level request."
    )
    ...
    # ========================================================================
    print("\n[STEP 4] Verifying Expected Outcome")
    print("-" * 70)
    reporter.add_step(
        "Step 4: Verify Expected Outcome",
        "Check that Max Defrost remains active and the CabHeatManStatus reflects the new heat level."
    )
    ...
    # Ensure Max Defrost related signals are still in correct state
    checks_passed &= check_can_signal(hil_var, "ClimateAirDistStatus_Defrost", 1)
    checks_passed &= check_can_signal(hil_var, "ClimateAirDistStatus_Floor", 0) # Negative check
    checks_passed &= check_can_signal(hil_var, "ClimateAirDistStatus_Vent", 0)   # Negative check
    ...
    # Generate report
    report_path = reporter.generate_html("test_max_defrost_heat_level_change_report.html")

    print(f"\nDry Run Report: {report_path}")
    print("\n[!] To run for REAL:")
    print("   1. Review the dry run report")
    print("   2. Verify all signals are correct")
    print("   3. Run: pytest -v test_max_defrost_heat_level_change.py")
    print("="*70 + "\n")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*70)
    print("[DRY RUN] MAX DEFROST HEAT LEVEL CHANGE DRY RUN")
    print("="*70)
    print("\nThis will:")
    ...
    _, _, _, hil_var = read_project_config()
    test_max_defrost_heat_level_change(hil_var)