...
def simulate_hardware_response(signal_name, expected_value):
    ...
    response_map = {
        ...
        "ClimatePowerStatus": 1 if simulated_state.get("MaxDefrostRequest", 0) == 1 else simulated_state.get("ClimatePowerRequest", 0), # ACRequest -> ClimatePowerRequest
        ...
        "CabHeatManStatus": 10 if simulated_state.get("MaxDefrostRequest", 0) == 1 else simulated_state.get("CabTempRequest", 0), # CabTempRequest maps to CabHeatManStatus
    }
...
def test_max_defrost_activation_scenario(hil_config):
    """
    ...
    reporter = TestReporter(
        ...
        "Verify Max Defrost activation via Driver Experience System request in simulation mode."
    )
    ...
    # Pre-conditions:
    reporter.add_step("Step 1: Set Pre-conditions", "Set VehicleMode to 'Running' (6) and ensure MaxDefrostRequest is off.")
    set_can_signal(hil_var, "VehicleMode", 6)  # VehicleMode to 'Running'
    set_can_signal(hil_var, "MaxDefrostRequest", 0) # Ensure Max Defrost is off initially
    set_can_signal(hil_var, "ClimatePowerRequest", 0) # Ensure AC is off initially
    set_can_signal(hil_var, "CabTempRequest", 0) # Ensure Cab Temp is off initially
    set_can_signal(hil_var, "AirRecirculationRequest", 1) # Set to Recirculation initially
    set_can_signal(hil_var, "ClimateAirDistRequest_Defrost", 0) # Ensure Defrost is off
    set_can_signal(hil_var, "ClimateAirDistRequest_Floor", 1) # Set Floor on
    set_can_signal(hil_var, "ClimateAirDistRequest_Vent", 1) # Set Vent on
    set_can_signal(hil_var, "HVACBlowerRequest", 1) # Set Blower to low

    asyncio.run(asyncio.sleep(0.5))

    # Trigger:
    reporter.add_step("Step 2: Trigger Max Defrost", "Set MaxDefrostRequest to 1 (On).")
    set_can_signal(hil_var, "MaxDefrostRequest", 1)  # MaxDefrostRequest to 1 (On)

    asyncio.run(asyncio.sleep(1)) # Wait for system to react

    # Expected Outcome:
    reporter.add_step("Step 3: Verify Expected Outcomes", "Check all relevant status signals after Max Defrost activation.")
    checks_passed = True
    checks_passed &= check_can_signal(hil_var, "MaxDefrostStatus", 1)  # MaxDefrostStatus == 1 (On)
    checks_passed &= check_can_signal(hil_var, "ClimatePowerStatus", 1)  # ACRequest == 1 (On) (mapped to ClimatePowerStatus)
    checks_passed &= check_can_signal(hil_var, "CabHeatManStatus", 10)  # CabTempRequest == Max (10) (mapped to CabHeatManStatus)
    checks_passed &= check_can_signal(hil_var, "AirRecirculationStatus", 0)  # AirRecirculationRequest == 0 (Off/Fresh Air)
    checks_passed &= check_can_signal(hil_var, "ClimateAirDistStatus_Defrost", 1)  # ClimateAirDistRequest_Defrost == 1 (On)
    checks_passed &= check_can_signal(hil_var, "ClimateAirDistStatus_Floor", 0)  # ClimateAirDistRequest_Floor == 0 (Off)
    checks_passed &= check_can_signal(hil_var, "ClimateAirDistStatus_Vent", 0)  # ClimateAirDistRequest_Vent == 0 (Off)
    checks_passed &= check_can_signal(hil_var, "HVACBlowerLevelStat_BlowerLevel", 10)  # HVACBlowerRequest == Max (10)

    # TODO: Signal Flap_Defrost not found in project config; manual verification required.
    # TODO: Signal Flap_Foot not found in project config; manual verification required.
    # TODO: Signal Flap_Center_Vent not found in project config; manual verification required.

    # Teardown (Optional: Resetting signals after test)
    reporter.add_step("Step 4: Teardown", "Reset MaxDefrostRequest to 0.")
    set_can_signal(hil_var, "MaxDefrostRequest", 0) # Turn off Max Defrost

    asyncio.run(asyncio.sleep(0.5))

    reporter.add_step("Step 5: Verify Teardown", "Verify MaxDefrostStatus is 0 after reset.")
    ...
    print("   4. Run: pytest -v your_test_file.py")
    print("="*70 + "\n")

    # Always pass in dry run (we're just validating logic)
    assert checks_passed # In dry run, we assert that the simulation logic passed, not actual hardware.
...
if __name__ == "__main__":
    ...
    _, _, _, hil_var = read_project_config()
    test_max_defrost_activation_scenario(hil_var)